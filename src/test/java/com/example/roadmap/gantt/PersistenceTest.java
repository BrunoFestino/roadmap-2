...
class PersistenceTest {
    @Test void migrationsPreservePlansStacksAndAbsences() throws Exception {
        try (var postgres = EmbeddedPostgres.start()) {
            ...
            schedules.saveSchedule("TTAR-TEST", start, start.plusDays(7), TaskStack.FRONTEND);
            var absence = absences.add("bfestino", start, start.plusDays(1), AbsenceType.VACATION, "Prueba");
            assertThat(flyway.migrate().migrationsExecuted).isZero();
            assertThat(new TargetStartRepository(jdbc).findScheduleByIssueKey("TTAR-TEST"))
                    .hasValue(new TargetStartRepository.Schedule(start, start.plusDays(7), 3, TaskStack.FRONTEND));
